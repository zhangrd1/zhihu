...
值得一提，它支持Unity中BiRP已有的资源类型，使得从传统渲染管线迁移到SRP变得相对容易。例如，内置渲染管线中，着色器主要使用的是CG，在Scriptable Render Pipeline（SRP）——包括URP和HDRP——中，Unity逐步转向使用HLSL（High-Level Shading Language）
...
# 04UE渲染架构的原则与方案
UE的渲染架构设计理念并不是以完全模块化为核心，而是以“统一集成、高质量默认配置与灵活定制”作为主导。与Unity SRP的“从底层完全可编程”的设计理念形成了对比。追求顶尖的视觉效果

UE的线程
- 游戏线程（Game Thread）（也称作主线程）
	- 负责资源加载合场景搭建
- 渲染线程（RenderingThread），
    - 专门用于生成渲染指令 (FRenderCommand) 和渲染逻辑的独立线程
    - 渲染线程负责剔除、渲染管线
    - Renderer层增强了光线追踪，特别是屏幕空间的光线追踪，加强了距离场的各种应用，同时删除了LPV。
- RHI线程（RHI Thread）
	- RHI（Render Hardware Interface）封装了众多图形API，负责生成具体的绘制指令提交GPU渲染，转换渲染指令到指定图形API，创建、上传渲染资源到GPU
...
UE5对渲染架构进行了革命性的更新，主要体现在两个关键技术上：
...
Unreal Engine (UE) 的渲染架构设计是高度模块化和面向扩展的，主要由多个层次和模块组成。其设计哲学强调可扩展性、高性能以及对复杂渲染需求的支持。为了深入理解UE的渲染架构，以下是它的主要组成部分、层次关系及设计哲学的分析。

1. 输入资源与资源管理
在渲染流程开始之前，各种资源（如网格、材质、纹理、光照信息）需要被正确加载和管理。UE的资源管理包括：

材质与Shader系统：材质（Material）在UE中是配置化的资源，结合了Shader代码、纹理、参数等。Shader代码主要由HLSL编写，通过材质编辑器（Material Editor）生成配置文件，最终被传递给渲染器使用。
FRHIResource：FRHIResource 是所有图形资源的基类，包括纹理、缓冲区等。它抽象了跨平台资源管理，使得渲染资源能够在不同图形API上保持一致性​ (Epic Games Developer)​ (Epic Games Developer)。
...
UE的渲染架构可以分为多个层次，从顶层的渲染控制逻辑到底层的图形API交互。


这部分负责渲染流程的总体调度和管理，包括场景管理、帧生成以及渲染器调度。主要由`FScene`, `FView`, `FSceneRenderer`等核心类构成。
...
- **FScene**：代表渲染场景，管理场景中的对象、光源、材质等。
- **FView**：代表渲染视图，管理摄像机参数、可见性、投影矩阵等信息。
- **FSceneRenderer**：负责渲染管线的调度和执行，是渲染流程的主要入口。不同的渲染器（如延迟渲染器、移动渲染器）都继承自该类，封装了具体的渲染流程。

#### **(2) 渲染管线层**

渲染管线层负责具体的渲染流程，通常由不同的渲染管线组成，如延迟渲染、前向渲染等。UE的渲染管线架构支持自定义和扩展。

- **FDeferredShadingSceneRenderer**：这是UE延迟渲染的核心实现，控制G-buffer的生成、光照计算、后处理等步骤。
- **Custom Render Passes**：UE允许开发者通过自定义渲染通道来插入或替换特定渲染阶段，提供了很大的灵活性。

在这个层次，UE的渲染管线通过多个阶段（Pass）组织，如阴影Pass、反射Pass、后处理Pass等。每个阶段可以被扩展、替换或自定义，实现高度定制化的渲染流程。

#### **(3) 中层抽象与API交互层**

这一层是渲染架构的核心抽象部分，它将高层的渲染流程与底层的图形API分离开来。UE的设计通过接口和抽象类，使得渲染管线与具体的图形API解耦。

- **FRHICommandList**：这是UE与底层图形API交互的主要接口。所有的渲染指令都会通过`FRHICommandList`发送到硬件。这种设计确保了UE能够支持多种图形API（如DirectX、Vulkan、Metal等），而无需在高层逻辑中进行更改。
- **FRHIResource**：代表渲染资源的抽象，如纹理、缓冲区等。它们被封装在这一层中，以确保跨平台一致性。

通过这种抽象，开发者可以在不影响上层逻辑的情况下更换底层图形API，并且可以在平台之间复用高层渲染逻辑。
...
UE的渲染架构层次之间通过清晰的接口和抽象类进行连接：

- **高层控制层**与**渲染管线层**之间通过`FSceneRenderer`接口进行交互。不同的渲染器（如延迟渲染器、前向渲染器）都可以通过继承并实现该接口，从而插入自定义的渲染逻辑。
- **渲染管线层**与**中层抽象层**之间通过`FRHICommandList`和`FRHIResource`等接口进行通信。渲染管线的各个Pass生成的渲染指令会通过这些接口发送到硬件。
...
UE的渲染架构采用了模块化、分层次的设计，确保了高性能和可扩展性。通过清晰的层次划分和接口抽象，UE实现了对不同渲染需求和图形API的支持。每一层都独立且高度模块化，彼此之间通过接口和抽象进行解耦，这种设计使得扩展渲染管线、定制渲染流程以及跨平台开发变得更加高效。

在Unreal Engine (UE) 的渲染架构中，FScene、FView 和 FSceneRenderer 是关键的类，它们分别负责管理场景数据、视图信息和渲染流程。以下是对这些类的深入分析：

1. FScene
FScene 是引擎中管理渲染场景的核心类，它负责存储场景中的所有对象，包括光源、几何体和材质等。它的主要职责包括：

场景数据的组织与管理：FScene 维护场景中所有可渲染对象的状态，并提供接口用于查询和访问这些对象。
渲染线程与游戏线程的同步：FScene 在游戏线程中被更新，但渲染线程需要读取场景数据，因此它也处理两者之间的同步工作。
此外，FScene 并不仅仅是一个数据容器，它还包含一些与场景管理相关的逻辑，例如灯光信息的管理和对象的可见性计算​ (Epic Games Developer)​ (GitHub)。

2. FView
FView 代表了一个渲染视图，是将3D场景投影到2D屏幕的关键。它包含了视图矩阵、投影矩阵、裁剪平面等信息，是渲染过程中必不可少的部分。具体来说：

视图设置与状态：FView 包含了视角、屏幕分辨率、场景可见性等数据，这些信息会影响场景的渲染方式。
投影计算：在渲染过程中，FView 的矩阵信息被用于将场景从世界坐标系转换到屏幕坐标系。
多视图支持：在一些复杂场景中（如VR或多摄像机视图），会有多个 FView 实例，每个实例对应一个独立的视图​ (Epic Dev | Home)​ (Epic Games Developer)。
3. FSceneRenderer
FSceneRenderer 是整个渲染流程的核心调度器。它负责组织和执行场景的渲染过程，不同的渲染器（如延迟渲染器、前向渲染器）都继承自这个类。其主要功能包括：
...
渲染任务的调度与优化：FSceneRenderer 还处理渲染任务的排序和优化，如批处理、透明度处理等，以确保高效渲染​ (GitHub)​ (Epic Games Developer)。
总结
FScene 负责管理和存储场景中的数据，提供渲染过程中所需的基础信息。
FView 提供了渲染所需的视图和投影信息，是场景到屏幕转换的核心。
FSceneRenderer 则是渲染的调度中心，负责组织和执行渲染流程。
这些类之间的关系层次分明，确保了渲染流程的可扩展性和灵活性。在UE的架构中，开发者可以通过扩展这些类来实现自定义的渲染效果和流程。

1. FDeferredShadingSceneRenderer
FDeferredShadingSceneRenderer 是Unreal Engine中用于实现延迟渲染（Deferred Shading）的核心渲染器类。延迟渲染是一种广泛用于现代游戏引擎的渲染技术，它首先将场景的几何信息存储在G-buffer中，然后在一个独立的阶段中执行光照计算。这种技术允许引擎高效地处理大量动态光源。FDeferredShadingSceneRenderer类的职责和工作流程包括以下几个方面：

渲染流程的核心控制：在每一帧渲染中，FDeferredShadingSceneRenderer会通过调用其Render函数来执行整个延迟渲染过程。这个过程包括从生成G-buffer到最终光照计算和后处理的所有阶段。
渲染通道的管理：FDeferredShadingSceneRenderer通过一系列的渲染Pass组织渲染工作，比如阴影Pass、光照Pass和后处理Pass等。这些Pass按照特定顺序执行，以确保最终的图像输出。
与Post Processing集成：FDeferredShadingSceneRenderer还负责管理和触发后处理效果（Post Processing），如景深、运动模糊和色调映射等。这些效果通常在渲染完场景的基础光照之后执行​ (玄冬Wong)​ (Unreal Art Optimization)。
2. Custom Render Passes（自定义渲染通道）
Unreal Engine提供了灵活的接口，允许开发者创建和插入自定义的渲染通道（Custom Render Passes），从而实现特定的视觉效果或处理需求。自定义渲染通道通常用于在默认的渲染流程中插入额外的渲染逻辑，比如特效、屏幕空间效果或调试工具。关键点包括：

渲染流程的扩展：开发者可以在现有渲染通道之前、之后甚至在中间插入自定义的Pass。例如，可以在所有后处理效果之前插入一个自定义Pass，用于计算额外的屏幕空间效果​ (Epic Dev | Home)。
全局Shader与自定义绘制：通过使用全局Shader和低级渲染接口（如FRHICommandList），开发者可以在自定义Pass中实现复杂的效果。比如，可以在特定渲染阶段执行额外的几何绘制，或处理特定的纹理效果​ (GitHub)​ (Epic Developer Community Forums)。
API支持与灵活性：引擎为自定义渲染通道提供了灵活的API支持，开发者可以通过继承或重写现有的渲染类，插入自定义逻辑。具体实现方式通常包括注册新的渲染通道，并在渲染流程中动态调用这些通道​ (玄冬Wong)。
总结
Unreal Engine的渲染架构设计具有很强的扩展性，通过FDeferredShadingSceneRenderer和Custom Render Passes，开发者可以精细控制渲染流程并添加自定义效果。这种设计既支持复杂的光照和后处理，又提供了在现有流程中插入特定逻辑的能力，极大地增强了引擎的灵活性和可定制性。

在Unreal Engine的渲染架构中，FRHICommandList 和 FRHIResource 是底层图形API交互的核心组件。它们通过抽象不同平台的图形接口，实现了跨平台一致的渲染流程，同时提供了灵活的资源管理和指令控制。

1. FRHICommandList
FRHICommandList 是一个关键的抽象，用于在渲染过程中调度和管理图形命令。这些命令可以包括绘制调用、资源绑定、状态设置等。其主要作用包括：
...
资源过渡管理：FRHICommandList 还管理资源状态的转换（如从读到写的转换），确保资源在不同阶段被正确使用​ (GitHub)​ (Epic Games Developer)。
2. FRHIResource
FRHIResource 是所有RHI（Render Hardware Interface）资源的基类，包括纹理、缓冲区、渲染目标等。它为图形资源提供了一层抽象，使得这些资源可以在不同的底层API（如DirectX、Vulkan、Metal等）之间保持一致的接口。其主要职责包括：
...
3. 虚幻引擎的高动态渲染（High-Level Rendering Abstractions）
UE在渲染架构中引入了一些高层次的抽象，以方便开发者进行渲染定制和高级功能扩展。
...
跨平台渲染抽象：RHI封装了不同平台（如DirectX、Vulkan、Metal等）的图形API，使得高层代码可以无缝跨平台执行。每个平台都有一个特定的RHI实现，RHI模块负责将统一的渲染命令转换为平台特定的API调用。
动态RHI选择：UE可以在运行时动态选择合适的RHI实现，基于平台和硬件特性做出优化选择。这种灵活性使得UE在不同平台上都能充分发挥硬件性能。
5. 渲染扩展点与插件化架构
UE提供了丰富的渲染扩展点，开发者可以通过插件系统定制引擎渲染功能。
...
HLSL与自定义着色器
HLSL着色器编写：在材质系统中，UE允许开发者通过HLSL编写自定义着色器逻辑，并将其集成到渲染管线中。这在需要实现特定视觉效果（如特殊的光照模型、体积效果）时非常有用。自定义着色器可以在材质编辑器中被调用，或者通过插件系统直接注入到渲染流程中。
