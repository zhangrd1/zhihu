...
核心渲染流程在渲染线程中执行，包含多个渲染通道，不同的绘制任务会被按照特定顺序处理。其中，核心类是`FSceneRenderer`。
- **FSceneRenderer**：负责调度和执行一系列Pass，是UE中渲染流程的真正实现。不同的渲染器都继承自该类，封装了具体的渲染流程，支持如延迟渲染、前向渲染等不同的渲染管线

其中，`FDeferredShadingSceneRenderer`是最关键的，实现了延迟渲染。
- **FMobileSceneRenderer**: 针对移动平台进行优化，专注于性能和功耗平衡，使用简化的渲染路径和特定的优化手段。

Deferred Rendering is the default in UE for high-quality scenes, allowing complex lighting calculations by deferring light processing until after all geometry is rendered.
Forward Rendering is used primarily for VR and other performance-critical scenarios, offering lower overhead at the expense of fewer dynamic light sources.
...
`FDeferredShadingSceneRenderer` 的数据主要来自于场景数据、渲染视图信息以及物体的几何和材质数据。
...
FDeferredShadingSceneRenderer通过 Mesh Processors 生成对应的 Mesh Draw Commands​。

这些 `FMeshDrawCommand` 会被组织成一个优化的列表，以供 GPU 高效处理。这个列表会根据渲染Pass（如Base Pass、Shadow Pass）进行分类，并在每个Pass中按需调用。


Mesh Processors 是生成 Mesh Draw Commands 的核心模块。它们根据不同的渲染路径（例如前向渲染或延迟渲染）将网格数据转换成适合执行的绘制命令。
...
Mesh Draw Commands 是渲染指令的抽象，包含了绘制每个物体所需的所有信息，如顶点缓冲区、索引缓冲区、材质、着色器等。每个网格对象在场景中都会生成一个或多个 Mesh Draw Commands，这些命令由 FMeshPassProcessor 处理并最终提交给渲染管线。这些命令的设计使得相似的绘制任务可以批量处理，减少 CPU 和 GPU 之间的开销​ (CNBlogs)​ (Dream Fields)。
...
在UE的渲染架构中，FPrimitiveSceneProxy 和 FMeshBatch 是两个关键的数据结构，负责从游戏逻辑层到渲染层的数据传递与处理。下面详细介绍这两个数据结构及它们的作用和关系。

1. FPrimitiveSceneProxy
FPrimitiveSceneProxy 是渲染线程中用于表示场景中可渲染对象（如网格体、光源等）的数据结构。它的主要职责是从游戏线程接收UPrimitiveComponent的信息，并将其转换为渲染线程可以处理的格式。

主要功能：
数据同步：在游戏线程中，每个场景对象通过UPrimitiveComponent进行描述。当这些组件被提交到渲染线程时，它们的副本会被封装为FPrimitiveSceneProxy。这个结构确保游戏线程和渲染线程之间的数据一致性。
可见性判断：FPrimitiveSceneProxy 负责处理场景中的可见性计算（如视锥裁剪、遮挡剔除等），并决定哪些对象需要在当前帧中被渲染。
网格与材质信息：该结构还存储了与对象关联的网格信息、材质、光照参数等。它提供了用于渲染的几何体信息，包括网格数据的批次和细节级别（LOD）​ (CNBlogs)​ (CNBlogs)。
2. FMeshBatch
FMeshBatch 是 UE 渲染架构中的核心数据结构，表示一个可渲染的网格。它包含了渲染一个物体所需的所有信息，例如顶点、索引缓冲、材质、着色器等。与FPrimitiveSceneProxy不同，FMeshBatch 更专注于具体的渲染过程。

主要功能：
解耦渲染任务：FMeshBatch 解耦了渲染对象和具体的渲染通道（Pass），这意味着一个FMeshBatch可以在多个不同的渲染通道中复用，例如深度预处理（DepthPass）和基础光照通道（BasePass）。
批次渲染优化：FMeshBatch 和FMeshBatchElement通过批处理减少Draw Call次数，从而提高渲染性能。它们支持实例化渲染和LOD切换，确保在不同的场景复杂度下，渲染效率得以最大化​ (CNBlogs)。
结构细节：
基本结构：
FMeshBatchElement：这是FMeshBatch的子结构，包含具体的网格数据，如顶点缓冲、索引缓冲、实例化信息等。每个FMeshBatch可以包含多个FMeshBatchElement，以支持复杂的几何体和多种渲染模式。
渲染参数：包括材质、着色器、剔除信息、实例化数据等，这些信息被传递到GPU进行实际渲染。
缓存与命令生成：在UE4.22的更新中，渲染架构引入了FMeshDrawCommand，通过缓存生成的绘制命令，减少每帧重复的命令生成，从而显著优化性能​ (CNBlogs)。
3. FPrimitiveSceneProxy 与 FMeshBatch 的关系
数据传递链：在渲染过程中，FPrimitiveSceneProxy 作为场景中物体的抽象代理，负责从游戏线程获取并管理该物体的渲染信息。而FMeshBatch 则进一步将这些信息拆解为具体的渲染批次，并提交给渲染管线执行。
模块化与可扩展性：这种设计使得UE的渲染系统非常灵活。通过FPrimitiveSceneProxy和FMeshBatch的解耦，不同的渲染通道可以复用相同的网格数据，而不必为每个渲染任务重新构建数据。
总体来说，FPrimitiveSceneProxy和FMeshBatch的协作使得UE的渲染架构具备了高度的模块化与扩展性，为高效渲染大规模场景提供了基础。

在Unreal Engine渲染架构中，FPrimitiveSceneProxy、FMeshBatch、FScene 和 FViewInfo 这四个核心数据结构紧密相关，它们共同构成了渲染流程中的不同阶段，协同完成场景的绘制。

1. FScene 和 FPrimitiveSceneProxy
FScene 是整个场景的抽象表示，负责存储场景中的所有可渲染对象、光源、环境光照、反射等信息。它可以被看作是场景的全局数据集。每一个FPrimitiveComponent（例如静态网格组件）在添加到场景时，都会生成一个对应的FPrimitiveSceneProxy，并将其注册到FScene中。FPrimitiveSceneProxy负责具体描述该对象在渲染线程中的数据，包括几何信息、材质、着色器等​ (CNBlogs)​ (CNBlogs)。

关系总结：

FScene 存储所有场景数据，是全局管理类。
FPrimitiveSceneProxy 是具体对象的渲染代理，被FScene管理和调度。
2. FViewInfo 和 FMeshBatch
FViewInfo 表示从特定视角（相机、玩家视角等）观察场景时的渲染数据。每个FViewInfo包含视锥、投影矩阵、可见性信息等，并且会基于这些信息决定哪些FPrimitiveSceneProxy需要被渲染。渲染过程会遍历场景中的FPrimitiveSceneProxy，并生成相应的FMeshBatch用于具体的绘制操作​ (CNBlogs)。

关系总结：

FViewInfo 决定了从哪个视角观察场景，进而决定可见的对象。
FMeshBatch 是实际用于渲染的指令集，由FPrimitiveSceneProxy在当前视图中生成并被FViewInfo调度执行。
3. 四者之间的关系与流程
FScene 作为场景的全局容器，包含所有场景数据。
...
4. 关键点总结
FScene 是全局场景数据管理者，包含所有FPrimitiveSceneProxy。
FPrimitiveSceneProxy 作为具体对象的渲染代理，与FMeshBatch协作提供绘制信息。
FViewInfo 决定了当前视角下的可见性，并驱动从FPrimitiveSceneProxy生成FMeshBatch，从而实现高效渲染。
这种架构设计保证了渲染数据的分离与高效调度，同时通过多个数据结构的解耦和协作，能够灵活应对不同的渲染需求和场景复杂度​ 
