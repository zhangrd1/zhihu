...
在游戏线程，场景中的可渲染对象通过UPrimitiveComponent进行描述，当被提交到渲染线程时会生成FPrimitiveSceneProxy对象作为副本。

FMeshDrawCommand是渲染指令的抽象，包含了绘制每个物体所需的所有信息，如顶点缓冲区、索引缓冲区、材质、着色器等。每个网格对象在场景中都会生成一个或多个 Mesh Draw Commands。


FMeshDrawCommand 是 Unreal Engine 渲染管线中一个核心的数据结构，主要用于组织和存储渲染命令，以优化渲染性能并支持复杂场景的高效渲染。它的引入是为了应对现代 GPU 的需求，特别是在需要处理大量静态和动态网格渲染的情况下，通过命令的缓存和复用显著减少渲染开销。

1. FMeshDrawCommand 的背景与作用
在传统的渲染管线中，每一帧都需要为每个物体生成新的渲染指令。UE4 引入了 FMeshDrawCommand，通过将绘制指令与具体的渲染状态、资源绑定等独立开来，使得命令可以缓存并在后续帧中复用。
...
2. FMeshDrawCommand 的工作流程
FMeshDrawCommand 的构建流程通常发生在场景初始化或对象加入场景时，尤其是对于静态网格，这些命令可以被缓存并在后续帧中直接复用。对于动态对象，每一帧都会更新和生成新的 FMeshDrawCommand。

流程解析：
命令生成：当 FMeshPassProcessor 处理 FMeshBatch 时，会根据渲染通道的需求生成 FMeshDrawCommand。这些命令会被关联到相应的渲染管线阶段，如 Base Pass、Depth Pass 等。
...
4. 命令优化与性能
缓存机制：通过在 AddToScene 阶段缓存 FMeshDrawCommand，UE4 能在后续帧直接复用这些命令，尤其在处理大量静态物体时，能够极大减少 Draw Call 数量和 CPU 负担。
并行指令处理：使用 FParallelMeshDrawCommandPass，这些命令可以在多个线程中并行生成和处理，充分利用 CPU 的多核优势。
5. 应用场景
静态物体：在大部分情况下，静态物体的渲染命令只需要在场景初始化时生成一次，后续帧中可以直接复用，带来显著的性能提升。
动态物体：尽管动态物体的命令需要每帧重新生成，但得益于 FMeshDrawCommand 的设计和缓存机制，即便是复杂场景中的大量动态物体，渲染效率也能得到保障。
总结
FMeshDrawCommand 是 UE4 渲染管线中的关键组成部分，通过对渲染命令的抽象、缓存和并行处理，显著优化了现代游戏中的渲染性能。它使得引擎能够在高效利用硬件资源的同时，应对复杂且动态的游戏场景，从而在不同的平台上提供一致的高性能渲染体验。


在UE4.22中引入了FMeshDrawCommand。通过缓存绘制命令减少每帧重复的命令生成。这些 `FMeshDrawCommand` 会被组织成一个优化的列表，以供 GPU 高效处理。这个列表会根据渲染Pass（如Base Pass、Shadow Pass）进行分类，并在每个Pass中按需调用。

Mesh Draw Commands 是渲染指令的最终形式，封装了每个绘制命令所需的所有信息：
- **几何数据**：如顶点缓冲区、索引缓冲区等。
- **材质和着色器**：包括所有的材质参数、绑定的Shader以及对应的渲染状态。
- **绘制参数**：如实例化信息、渲染顺序、剔除结果等。

FMeshBatch表示一个可渲染的网格，包含了渲染物体所需的所有信息，例如顶点、索引缓冲、材质、着色器等。FMeshBatchElement是FMeshBatch的子结构，包含具体的网格数据，如顶点缓冲、索引缓冲、实例化信息等。每个FMeshBatch可以包含多个FMeshBatchElement，以支持复杂的几何体和多种渲染模式。每个元素描述了物体在特定 LOD、特定材质下的渲染细节。这种设计允许引擎高效地处理实例化渲染、多级细节渲染（LOD）等场景。
而FMeshBatch 则进一步将这些信息拆解为具体的渲染批次，并提交给渲染管线执行。不同的渲染通道可以复用相同的网格数据，而不必为每个渲染任务重新构建数据。

FMeshBatch 解耦了渲染对象和具体的Pass，这意味着一个FMeshBatch可以在多个不同的Pass中复用，例如DepthPass和BasePass。

FMeshBatch 和FMeshBatchElement通过批处理减少Draw Call次数，从而提高渲染性能。它们支持实例化渲染和LOD切换。


渲染参数：包括材质、着色器、剔除信息、实例化数据等，这些信息被传递到GPU进行实际渲染。


FPrimitiveSceneProxy包含所有的渲染数据，如网格信息、材质、光照参数等。
- 为不同类型的组件（如静态网格、动态网格、骨骼网格等）提供自定义的渲染逻辑

- "FPrimitiveSceneProxy"负责通过对"GetDynamicMeshElements"和"DrawStaticElements"的回调将FMeshBatch提交给渲染器。不同类型的资源有不同的 FPrimitiveSceneProxy 子类。包含了特定类型渲染相关的信息，如几何体的顶点和索引数据、材质和纹理、光照和阴影信息等。

  "FMeshBatch"将"FPrimitiveSceneProxy"实现（用户代码）与网格体通道（私有渲染器模块）解耦。它包含了通道确定最终着色器绑定和渲染状态所需的所有内容，如存储顶点相关数据的 FVertexFactory 以及材质渲染需要的 FMaterialRenderProxy 等。 FMeshBatch 也是网格绘制处理器 FMeshPassProcessor 及处理过程中的重要元素，存储在 FPrimitiveSceneInfo::StaticMeshes 中。
...
作者：MarsZhou
链接：https://zhuanlan.zhihu.com/p/657669302
来源：知乎
...
`GetDynamicMeshElements()` 是FPrimitiveSceneProxy的一个方法，用于生成可渲染的FMeshBatch。FMeshBatch存着一组FMeshBatchElement的数据，这组FMeshBatchElement的数据拥有相同的材质和顶点缓冲。FMeshBatchElement是单个网格模型的数据，包含网格渲染中所需的部分数据，如顶点、索引、UniformBuffer及各种标识等。
- 主要作用是收集场景中动态的几何数据，并将其传递给渲染通道。
...
FPrimitiveSceneProxy 负责处理场景中的可见性计算，并决定哪些对象需要在当前帧中被渲染。
...
Views：表示当前渲染的视图（例如主相机、反射探针等）。
VisibilityMap：用来标记哪些视图中的物体可见。
Collector：用于收集生成的 FMeshBatch，这些批次随后会在渲染阶段使用。
GetDynamicMeshElements() 的实现通常会根据视图和可见性信息生成多个 FMeshBatch，并将这些批次添加到 Collector 中。例如，在动态阴影渲染中，该函数会根据每个灯光源、相机视角等参数生成不同的渲染批次。

通过 FPrimitiveSceneProxy 获取该物体的变换矩阵、材质等数据。遍历视图数组，根据 VisibilityMap 确定哪些视图需要渲染该物体。创建并配置 FMeshBatch，包括顶点工厂、材质代理、渲染状态等。将配置好的 FMeshBatch 添加到 Collector 中。
这意味着，对于每一个动态物体（如角色、移动的物体等），在每一帧的渲染准备阶段，都会执行这一流程。


FMeshBatch 是 GetDynamicMeshElements() 的核心输出，它定义了具体的渲染指令。FMeshBatch 是一个轻量级结构体，其中包含了渲染一个物体所需的全部信息：
...
3. 多线程与缓存机制
在 UE 的现代渲染管线中，GetDynamicMeshElements() 的输出可以被缓存以提高性能。对于静态物体，渲染指令通常在场景加载时生成并缓存，而动态物体则需要在每一帧重新生成。这种缓存机制在 FPrimitiveSceneProxy 和 FMeshDrawCommand 系统之间进行协调​(
CNBlogs
,
CNBlogs
)。
...
4. 实际应用场景
动态物体的渲染：角色或其他动态物体需要在每一帧通过 GetDynamicMeshElements() 生成新的 FMeshBatch。这部分数据会被动态地传递给 GPU。
阴影、反射等效果：同一个物体在不同的渲染通道中可能需要不同的批次（如阴影通道、反射通道等），这也是通过 GetDynamicMeshElements() 生成的。
总结
GetDynamicMeshElements() 是动态物体渲染的关键，它将场景中每个可见物体的渲染数据转换为 FMeshBatch，并结合 UE 的并行和缓存机制，使得引擎能够高效地处理复杂场景。

1. 场景数据的准备
涉及 FPrimitiveSceneProxy 和 FMeshBatch 等核心类。这些数据结构用于表示场景中的物体，并包含渲染所需的所有信息。例如，FPrimitiveSceneProxy 是从游戏线程到渲染线程的镜像数据，而 FMeshBatch 则封装了渲染所需的所有 Mesh 数据



2. 渲染命令的缓存与构建
为了提升性能，UE4.22 之后重构了 Mesh 渲染管线，引入了 FMeshDrawCommand 和 FMeshPassProcessor 等概念。这些改进使得引擎可以在场景加入时就缓存绘制指令，从而避免每帧重新生成命令，减少 Draw Call 数量。。

3. 渲染通道与任务分配
渲染管线中的渲染通道（Pass）是通过 FMeshPassProcessor 进行组织的。根据具体需求，渲染通道可分为标准通道、手动通道和动态绘制通道。标准通道（如深度通道、基通道等）可以进行命令缓存和并行处理，而动态通道适用于不需要高性能优化的场景，如编辑器中的实时预览

4. 延迟渲染和其他优化
UE 的延迟渲染管线通过多次 Pass 处理（如 GBuffer 和 Lighting Pass），有效处理复杂场景中的多光源计算。同时，UE 还支持基于瓦片的延迟渲染（TBDR），进一步减少无效计算，提升渲染效率。对于某些特定平台，UE 还实现了诸多优化变种

5. 并行渲染与任务调度
在渲染的核心部分，UE 采用了并行渲染的架构，利用多线程和 RDG（Rendering Dependency Graph）来调度和执行渲染任务。这使得在复杂场景下，可以充分利用多核 CPU 和现代 GPU 的计算能力，实现高效的实时渲染。
...
网格体渲染从FPrimitiveSceneProxy（图元场景代理）开始，将FMeshBatch提交给渲染器，是UPrimitiveComponent在渲染器的代表，镜像了UPrimitiveComponent在渲染线程的状态。
...
2. FMeshPassProcessor 的作用
FMeshPassProcessor 是负责将FMeshBatch转换为渲染命令的核心类。每个渲染通道（如 Base Pass、Depth Pass、Shadow Pass 等）都有一个相应的 FMeshPassProcessor，该类负责处理与该通道相关的所有 FMeshBatch，并生成最终的渲染命令。

批次的处理和分类：FMeshPassProcessor 会根据物体的特性（如是否需要写入深度缓冲、是否透明等）将 FMeshBatch 归类到不同的渲染通道。
渲染命令生成：在处理每个批次时，FMeshPassProcessor 会根据视图、材质和几何信息生成 FMeshDrawCommand，这些命令会被加入渲染任务列表，在后续阶段被执行。

UE 的渲染流程支持多种渲染通道，每种通道都有不同的渲染需求，例如：

Base Pass：负责处理场景的基本颜色、光照等。
Depth Pass：专门生成深度缓冲，用于阴影、遮挡剔除等。
Custom Passes：例如自定义后处理效果等。
在这些通道中，FMeshPassProcessor 负责调用适当的渲染逻辑并生成对应的渲染命令。


`FMeshPassProcessor::AddMeshBatch()` 将FMeshBatch添加到FMeshDrawCommand，用于将物体的渲染数据提交给特定的Pass。将批次数据转换为适合特定Pass的命令。这些命令最终会被放入渲染指令列表。

根据不同的渲染路径（例如前向渲染或延迟渲染）将网格数据转换成适合执行的绘制命令。
...
4. FParallelMeshDrawCommandPass 与命令调度
在复杂场景中，UE 通过 FParallelMeshDrawCommandPass 进行并行渲染调度。每个通道的渲染命令都会被调度到不同的线程中，并在最终阶段合并结果。这种设计利用了现代多核 CPU 的并行计算能力，提高了渲染效率​(
CNBlogs
)。
...
每一个FPrimitiveComponent（例如静态网格组件）在添加到场景时，都会生成一个对应的FPrimitiveSceneProxy，并将其注册到FScene中。FPrimitiveSceneProxy负责具体描述该对象在渲染线程中的数据，包括几何信息、材质、着色器等​ 

每个场景中的可渲染对象有对应的FPrimitiveSceneProxy，它负责将游戏线程的数据转换为渲染线程可用的数据。
